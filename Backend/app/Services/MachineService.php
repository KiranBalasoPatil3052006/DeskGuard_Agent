...
class MachineService
{
    ...
    {
        try {
            $query = Machine::with(['assignedUser', 'currentStatus'])
    ...
    /**
     * Get summary counts for a company's machines.
     *
     * @param  int   $companyId
    ...
    public function getCompanyMachineSummary(int $companyId): array
    {
        try {
            ...
            $online  = Machine::where('company_id', $companyId)->where('is_online', true)->count();
