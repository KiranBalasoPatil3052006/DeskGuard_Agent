...
namespace DeskGuardAgent.Collectors
{
    ...
    public class EventLogCollector : ICollector<List<EventLogInfo>>
    {
        ...
        private readonly ConcurrentDictionary<string, DateTime> _lastBookmark = new();
        ...
        public async Task<List<EventLogInfo>> CollectAsync()
        {
            ...
            try
            {
                ...
                foreach (var entry in eventList)
                {
                    ...
                    {
                        _lastBookmark.AddOrUpdate(entry.LogName, entry.TimeGenerated,
                            (key, existing) => entry.TimeGenerated > existing ? entry.TimeGenerated : existing);
                    }
                }
