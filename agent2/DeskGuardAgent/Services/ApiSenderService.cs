...
namespace DeskGuardAgent.Services
{
    ...
    public class ApiSenderService : IApiSenderService
    {
        ...
        /// Attempts to send synchronously with a short timeout; does not block shutdown.
        /// </summary>
        public async Task<bool> SendShutdownNotificationAsync()
        {
            try
            {
                var payload = new
                {
                    machine_uid = _settings.AgentId,
                    machine_name = Environment.MachineName,
                    event_type = "shutdown",
                    timestamp = DateTime.UtcNow
                };

                _logger.LogInformation($"Sending shutdown notification for agent: {_settings.AgentId}");
                return await SendPayloadAsync(payload, ApiRoutes.ShutdownEndpoint, "ShutdownNotification");
            }
            catch (Exception ex)
            {
                _logger.LogError("Failed to send shutdown notification", ex);
                return false;
            }
        }
        ...
        /// Called by the installer uninstaller before deleting files.
        /// </summary>
        public async Task<bool> SendUninstallNotificationAsync(string reason = "User uninstalled via installer")
        {
            try
            {
                var payload = new
                ...
                _logger.LogInformation($"Sending uninstall notification for agent: {_settings.AgentId}");
                return await SendPayloadAsync(payload, ApiRoutes.UninstallEndpoint, "UninstallNotification");
            }
            catch (Exception ex)
            {
                _logger.LogError("Failed to send uninstall notification", ex);
                return false;
            }
        }
