...
**3 New Migrations**:
- `2026_07_10_000001_add_severity_to_change_history` — Adds `severity` column (information/warning/important/critical) to `change_history`
- `2026_07_10_000002_create_security_baselines_table` — New `security_baselines` table with (machine_id, component) unique key
- `2026_07_10_000003_create_configuration_baselines_table` — New `configuration_baselines` table with (machine_id, setting_key) unique key

**2 New Models**:
- `SecurityBaseline` — Stores approved security posture (antivirus, firewall state)
- `ConfigurationBaseline` — Stores approved config state (services, startup programs)

**Services Updated**:
- `BaselineService` — Added `syncSecurityBaseline`, `syncConfigurationBaseline`, `getSecurityBaseline`, `getConfigurationBaseline`

**Controllers Updated**:
- `AgentChangeController` — Now stores severity, creates alerts for critical/important changes, sends email notifications via NotificationService
- `ChangeController` — Added `severity` query parameter filter (bySeverity scope)
- `BaselineController` — Added `securityBaseline`, `configurationBaseline`, `resyncSecurity`, `resyncConfiguration`, `approveChange` endpoints
...
- `POST /api/v1/changes/approve` — Approval workflow (updates baseline + resolves alerts)
