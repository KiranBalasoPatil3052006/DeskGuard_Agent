 ...
 * @property-read Collection<int, HardwareBaseline> $hardwareBaselines
 * @property-read Collection<int, SoftwareBaseline> $softwareBaselines
 * @property-read Collection<int, ChangeHistory> $changeHistories
 *
...
class Machine extends Model
{
    ...
    // PERFORMANCE: $appends removed to prevent N+1 queries.
    // Previously appended ['employee_name', 'current_user', 'cpu_model', 'ip_address', 'mac_address']
    // which triggered lazy-load queries for assignedUser and networkAdapters on EVERY
    // machine serialization. Frontend reads these from eager-loaded relations instead.
    // Accessors are kept below for explicit use when needed.
    ...
    {
        // PERFORMANCE: Guard with relationLoaded to prevent lazy-load N+1 query
    ...
    /**
     * Get the hardware baselines for the machine.
     *
     * @return HasMany<HardwareBaseline, $this>
     */
    public function hardwareBaselines(): HasMany
    {
        return $this->hasMany(HardwareBaseline::class);
    }

    /**
     * Get the software baselines for the machine.
     *
     * @return HasMany<SoftwareBaseline, $this>
     */
    public function softwareBaselines(): HasMany
    {
        return $this->hasMany(SoftwareBaseline::class);
    }

    /**
     * Get the change history records for the machine.
     *
