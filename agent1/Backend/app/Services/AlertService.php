...
class AlertService
{
     ...
     */
    public function getCompanyAlerts(int $companyId, ?string $severity = null, ?string $status = null, int $perPage = 50): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        try {
            $query = Alert::select(['id', 'company_id', 'machine_id', 'title', 'description', 'severity', 'status', 'acknowledged_by', 'resolved_by', 'created_at', 'updated_at'])
