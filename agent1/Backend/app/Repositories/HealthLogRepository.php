...
class HealthLogRepository extends BaseRepository
{
    ...
    public function getHistory(int $machineId, string $from, string $to): Collection
    {
        try {
            return $this->model
                ->select(['id', 'machine_id', 'company_id', 'cpu_percentage', 'cpu_temperature', 'ram_percentage', 'ram_used_bytes', 'ram_available_bytes', 'ram_total_bytes', 'disk_percentage', 'disk_free_bytes', 'disk_total_bytes', 'battery_percentage', 'network_sent_bytes', 'network_received_bytes', 'online_status', 'collected_at', 'created_at'])
                ->where('machine_id', '=', $machineId)
    ...
    public function getLatestByCompany(int $companyId, int $limit = 100): Collection
    {
        try {
            return $this->model
                ->select(['health_logs.id', 'health_logs.machine_id', 'health_logs.company_id', 'health_logs.cpu_percentage', 'health_logs.ram_percentage', 'health_logs.online_status', 'health_logs.collected_at', 'health_logs.created_at'])
                ->join('machines', 'machines.id', '=', 'health_logs.machine_id')
                ->where('machines.company_id', '=', $companyId)
                ->latest('health_logs.created_at')
                ->limit($limit)
                ->get();
        } catch (\Throwable $e) {
