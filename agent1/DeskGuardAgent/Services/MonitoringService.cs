    ...
    public class MonitoringService : IMonitoringService
    {
        ...
        public async Task ExecuteCollectionCycleAsync()
        {
            ...
            try
            {
                ...
                // Unchanged sections are omitted from the payload, reducing per-cycle
                // bandwidth for static fields (SystemInfo, Antivirus, etc.) to near-zero.
                ...
                var healthPayload = new HealthPayload
                {
                    ...
                    CpuInfo = cpu,
                    MemoryInfo = memory,
                    DiskInfo = isFirstCycle || !stableUnchanged ? disks : null,
                    BatteryInfo = battery,
                    NetworkInfo = isFirstCycle || !stableUnchanged ? network : null,
                    ...
                    PeripheralInfo = isFirstCycle || !stableUnchanged ? peripherals : null
                };

                // Step 4: Send the health payload to the backend API.
                await _apiSender.SendHealthPayloadAsync(healthPayload);
                ...
                if (hardwareTask.Result != null)
                {
                    ...
                    await _apiSender.SendHardwareInventoryAsync(hwPayload);
                ...
                if (softwareTask.Result != null)
                {
                    ...
                    await _apiSender.SendSoftwareInventoryAsync(swPayload);
        ...
        public async Task SendDeviceSyncAsync(List<PeripheralInfo> devices)
        {
            try
            {
                ...
                await _apiSender.SendDeviceSyncAsync(payload);
