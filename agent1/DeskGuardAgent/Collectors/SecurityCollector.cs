            ...
            try
            {
                await Task.Run(() =>
                {
                            ...
                            @"root\SecurityCenter2", "SELECT * FROM AntivirusProduct"))
                        {
                            using (ManagementObjectCollection results = searcher.Get())
                            {
                                foreach (ManagementObject obj in results)
                                {
                                    ...
                                    antivirusInfo.ProductVersion = obj["productUptoDate"]?.ToString() ?? "Unknown";
                    ...
                    catch (ManagementException)
                    {
                        ...
                        _logger.LogWarning("Security Center2 WMI namespace unavailable, checking Windows Defender status via MSFT_MpComputerStatus.");
