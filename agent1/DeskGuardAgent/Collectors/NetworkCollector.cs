                        ...
                        try
                        {
                                ...
                                $"SELECT BytesReceivedPersec, BytesSentPersec FROM Win32_PerfRawData_Tcpip_NetworkInterface WHERE Name = '{ni.Name.Replace("'", "''")}'"))
                            {
                                using (ManagementObjectCollection results = searcher.Get())
                                {
                                    ...
                                    {
                                        // Convert from perf counter raw values.
                                        networkInfo.BytesReceived = Convert.ToInt64(obj["BytesReceivedPersec"] ?? 0);
                                        networkInfo.BytesSent = Convert.ToInt64(obj["BytesSentPersec"] ?? 0);
                                    }
                                }
                            }
                        }
