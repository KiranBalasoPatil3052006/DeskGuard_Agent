...
import {
  ...
  FaExclamationTriangle, FaCheckCircle, FaDownload
} from 'react-icons/fa';
...
const AlertsList = () => {
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      ...
      setSummary({
        ...
        critical: d.critical_count || list.filter(a => a.severity === 'critical').length,
        warning: d.warning_count || list.filter(a => a.severity === 'warning').length,
        resolved: d.resolved_count || list.filter(a => a.status === 'resolved').length,
      });
    } catch (err) {
  ...
  }, [currentPage, severityFilter, statusFilter, searchFilter]);
  ...
  const handleResolve = async (alertId) => {
    setActionLoading(alertId);
    try {
      await resolveAlert(alertId, resolveNote);
      setAlerts(prev =>
  ...
  const getSeverityIcon = (severity) => {
    switch (severity) {
      case 'critical': return <FaExclamationCircle className="text-danger" />;
  ...
  return (
    <div className="container-fluid p-0">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button className="btn btn-outline-primary btn-sm" onClick={handleRefresh} disabled={isRefreshing}>
